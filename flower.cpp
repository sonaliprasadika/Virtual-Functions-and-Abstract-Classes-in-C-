...
Flower::Flower(const Vector2& center)
{
    ...
    pistil = std::make_unique<Circle>(Vector2(center.x(), center.y() - PISTIL_RADIUS / 2), PISTIL_COLOR, PISTIL_RADIUS);

    const Vector2 leaf_center(stem_center.x() - STEM_WIDTH / 2 + 1, stem_center.y() + STEM_HEIGHT / 4);
    leaf = std::make_unique<Triangle>(LEAF_COLOR,
...
{
    stem->draw(canvas); // Use -> instead of . to dereference and call the method on the pointed-to object
    pistil->draw(canvas);
    leaf->draw(canvas);
    for (int x = 0; x < PISTIL_RADIUS; x++)
    {
        ...
        drawPedals(canvas, pistil->center(), x, y); // Assuming center() is a method you've implemented or adjusted
    }
}
